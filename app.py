...
if analyze_button:
    if not selected_drugs:
        ...
        if len(risk_drugs) > 0:
            ...
            st.info("💡 해당 약품들은 인지저하 위험이 존재함에도 현행 DUR 병용금기 체계에서 경고해주지 않는 일반의약품입니다. 복용 시 주의가 필요합니다.")
            ...
            for index, row in risk_drugs.iterrows():
                with st.expander(f"💊 {row['제품명']} (성분명: {row['성분명']})", expanded=True):
                    ...
                    st.markdown("- 고령자의 경우 장기 복용을 피하고, 약사 또는 의사와 상담 후 복용하시기 바랍니다.")
                    
        # 위험 약물이 없는 경우
        else:
            st.success("✅ 선택하신 약품 조합에서는 알려진 인지저하(DICI) 위험 성분이 발견되지 않았습니다.")
            st.balloons()
